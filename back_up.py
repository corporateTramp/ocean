...
def start_init(urls, wait, db_data="dbname=alex user=alex password=1"):
	...
	for url in urls:
		...
		try:
			...
			fullName = data["entry_data"]["ProfilePage"][0]["user"]["full_name"] if fullName is not None else fullName = ""
			bio = data["entry_data"]["ProfilePage"][0]["user"]["biography"] if bio is not None else bio = ""
			external_url = data["entry_data"]["ProfilePage"][0]["user"]["external_url"] if external_url is not None else external_url = ""
			...
			if private == False:
				for i in range (0,9):
					...
					alt = data["entry_data"]["ProfilePage"][0]["user"]["media"]["nodes"][i]["caption"] if alt is not None else alt = ""	
