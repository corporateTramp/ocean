...
def start_init(urls, wait, db_data="dbname=alex user=alex password=alexdb"):
	...
	for url in urls:
		...
		try:
			...
			scan_sessions_add = (account_id, publications, subscribers, subscribtions, likes_sum/10, likes_sum/subscribers, publications/subscribers)
			cur = conn.cursor()
			cur.execute("INSERT INTO scan_sessions(account_id, publications, subscribers, subscribtions, created_at, active_subscribers, er, avg) VALUES (%s, %s, %s, %s, date_trunc('second',CURRENT_TIMESTAMP), %s, %s, %s);", scan_sessions_add)
			conn.commit()
			cur.close()
...
def create_tables(db_data="dbname=alex user=alex password=alexdb"):	
	...
	cur.execute("CREATE TABLE scan_sessions ( id SERIAL PRIMARY KEY, account_id integer, publications integer,subscribers integer, subscriptions integer, created_at timestamp, active_subscribers NUMERIC(4), er NUMERIC(4), avg NUMERIC(4));")
